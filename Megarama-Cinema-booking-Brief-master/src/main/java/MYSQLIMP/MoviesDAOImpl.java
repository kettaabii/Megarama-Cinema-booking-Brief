...
public class MoviesDAOImpl implements MovieDAO {
    ...
    @Override
    public List<Movie> getAllMoviesadmin() throws SQLException {
    ...
    @Override
    public Movie getMovieByIdadmin(int filmId) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    Movie movie = new Movie(filmId, movieTitle, description, releaseDate, duration, genre, director, cover);
                    movies.add(movie);
                }
            }
        }
